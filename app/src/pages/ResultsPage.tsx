...
import { getEvent, getClub, getUsers, userToMember } from '../services/pocketbase';
...
export const ResultsPage: React.FC = () => {
  ...
  const results = calculateBordaCount(wines, rankings);
  const maxPoints = wines.length * rankings.length;
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-8">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="type-headline-small" style={{ fontFamily: 'Playfair Display, serif', color: 'var(--md-on-surface)' }}>Results</h1>
        </div>
      ...
      {revealStep === 1 && !revealed && (
        <div className="card-elevated p-8 text-center cursor-pointer fade-in" onClick={doReveal} role="button"
          ...
          <p className="type-body-medium mb-6" style={{ color: 'var(--md-on-surface-variant)' }}>
            {rankings.length === 0
              ...
              : event.type === 'blind'
              ? `The wines have been judged blind by ${rankings.length} taster${rankings.length !== 1 ? 's' : ''}. Let's reveal the winner!`
              : `${rankings.length} taster${rankings.length !== 1 ? 's' : ''} have cast their votes. The winner is...`}
          </p>
      ...
      {/* WINNER */}
      {revealStep === 2 && winner && (
        <div className={`relative overflow-hidden text-white transition-all duration-700 ${revealed ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}
          style={{
        ...
        <div className={`transition-all duration-500 ${showPodium ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          <div className="space-y-2">
            {results.map((result: any, i: number) => {
              ...
              return (
                <div key={result.wineId} className="card-outlined flex items-center gap-3 fade-in"
                  style={{
                  ...
                  <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0" style={{ background: medal.bg }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="taste-bar-track mt-2">
                      <div className="taste-bar-fill" style={{ width: `${maxPoints > 0 ? (result.totalPoints / maxPoints) * 100 : 0}%`, background: medal.color }} />
                    </div>
                  </div>
                  ...
                  {results.map((result: any) => {
                    ...
                    return (
                      <tr key={result.wineId} style={{ borderTop: '1px solid var(--md-outline-variant)' }}>
                        ...
                        {members.map(member => {
                          const ranking = rankings.find((r: any) => r.memberId === member.id);
      ...
      {/* TASTING NOTES */}
      {showPodium && rankings.some((r: any) => r.notes && Object.keys(r.notes).length > 0) && (
        <div className="fade-in">
          ...
          <div className="space-y-3">
            {results.map((result: any) => {
              const notesForWine = rankings
                .filter((r: any) => r.notes?.[result.wineId])
                .map((r: any) => ({ member: members.find(m => m.id === r.memberId), note: r.notes![result.wineId] }))
              ...
              return (
                <div key={result.wineId} className="card-outlined p-4" style={{ borderRadius: 'var(--shape-large)' }}>
                  ...
                  <div className="space-y-3">
                    {notesForWine.map(({ member, note }: any) => (
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          {note.rating > 0 && (
                            <span className="flex items-center ml-auto">
                              {[...Array(note.rating)].map((_: any, i: number) => (
